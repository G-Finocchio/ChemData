...
- "Data_Analysis.R" runs our model with final parameters on the entire data and creates the excel table with the final SHIM coefficients and the prediction vs observed plot for these coefficients. (The table and plots are already in folder Results, but can be created again from scratch by running this file.) Running this file might take around 3 hours.  
- "Grid_search_local.R" contains an example on how grid search can be performed. 
- "Train_test_comparison.R" contains the prediction on test set with best Random forest model, Lasso model, and SHIM model. The RF and Lasso models are trained for best coefficients, while the best SHIM coefficients are loaded .(SHIM can also be trained from scratch, but this might take 2 hours.) The parameter tuning was done with cross validation on train set. 
...
-"SHIM_4way_CB.R" contains our continuous Bernoulli hierarchical model. It uses "Combinatorics.R", "functions_for_updates.R", "Updates.R" and "Metrics.R"
...
-It also contains the plots for predicted vs observed for Lasso, Random Forest, SHIM on test data and SHIM o entire data.

Information about running the code:

-Running "Final_resuts.R" and "Train_test_comparison.R" will produce all the results form the paper and the supplement of the paper.
