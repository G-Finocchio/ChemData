...
# Allows to do grid search 
# The best parameters are directly selected such that we directly train the optimal model.
...
# Directly load the coefficients obtained by using best parameters on train set
coefs_all<-readRDS(paste0("../coefficients/Coefficients_traintest_lmd_", 1e-5,"_", 1e-4,'_', 1e-3, ".rds"))
intercept_all<-coefs_all[1]
beta_all_shim<-coefs_all[-1]

pred.shim=kappa1(x.test%*%beta_all_shim + intercept_all)
r2(y.test, pred.shim)
ObservedYield <- as.vector(y.test)
