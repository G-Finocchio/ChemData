...
def mains_contribution(X,
                        beta_main,
                        l1=36, l2=3, l3=4):
    range_main = get_ranges3(l1, l2, l3)[0]  # first element
...
def two_ways_contribution(X,gamma_vec,
    ...
    range_2ways = get_ranges3(l1, l2, l3)[1]  # second element
...
def three_ways_contribution(X,
    ...
    range_3ways = get_ranges3(l1, l2, l3)[2]  # third element
...
def Q_bern(X,y,beta,gamma_vec,delta_vec, lambda_beta,lambda_gamma,lambda_delta, 
        w_beta,w_gamma=1,w_delta=1,l1=36,l2=3,l3=4,scaled=True,intercept=0):
    """
    Q Bernoulli penalized log likelihood
    """

    if len(beta) == l1 + l2 + l3:
        # Give beta for main and compute for the rest given gamma ...
        beta_2way = get_beta_vec_2way3(beta=beta,
                                    l1=l1, l2=l2, l3=l3, 
                                    gamma=gamma_vec,
                                    only_beta=False)
        beta_3way = get_beta_vec_3way3(beta_2way=beta_2way,
                                    l1=l1, l2=l2, l3=l3, 
                                    delta=delta_vec,
                                    only_beta=False)
        beta = np.concatenate([beta, beta_2way, beta_3way])

    # Penalty terms
    penalty_beta = get_penalty(vector=beta[get_ranges3(l1, l2, l3)[0]],
                            weights=w_beta,
                            lambda_=lambda_beta)
    ...
    loss = -log_like + penalty_beta + penalty_gamma + penalty_delta
    return loss
...
def minimizer_Q_bern_beta(X, Z, y, C, lambda_, beta_old, weight=1, scaled=True):
    def fct(b):
        penalty = abs(b) * lambda_ * weight
        v = X * b + Z * (b ** 2) + C
        log_like = np.sum(y * v - kappa0(v))
