...
class SHIM_2way_CB:
    def __init__(self, X, y, beta_init, gamma_init,  
                 l1=7, l2=8):
    ...
    def fit(self, X, y,
        ...
        beta_2way = get_beta_vec_2way2(beta=beta_hat,  l1=self.l1, l2=self.l2,  
                                       gamma=gamma_hat, only_beta=False)

        beta_all = np.concatenate([beta_hat, beta_2way])
        ...
        for it in range(1, max_iter + 1):
            ...
            # -- Update beta
            beta_hat = update_beta(X=X, y=y, beta_hat=beta_hat, gamma_hat=gamma_hat, lambda_beta=lambda_beta,
                                intercept=intercept_loc, l1=self.l1, l2=self.l2,  w=w_beta)
            ...
            if Q_new == Q_old:
                self.beta_hat, self.gamma_hat, = beta_hat, gamma_hat
                self.beta_all = beta_all
                self.intercept = intercept_loc
                print("The model has converged.")
                return {"beta_hat": self.beta_hat, "gamma_hat": self.gamma_hat, "beta_all": beta_all, "intercept": self.intercept}
