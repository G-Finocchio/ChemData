...
def irlasso_cb(X, Y, lambdas, w_lambda=None, beta0=None, centering=True, scaling=True, intercept=True, maxit=10, tol=0.0545, sd_tol=1e-6, verbose=False, C=0):
    ...
    while True:
        ...
        for m in nc:
            ...
            # Fit lasso with glmnet_python
            model = Lasso(alpha=lambdas[m],
                            fit_intercept=False,
                            weights=w_lambda)
            ...
            if intercept:
                s_lasso = np.where(model.coef_[1:] != 0)[0]
                ...
                s_lasso = np.where(model.coef_ != 0)[0]
                R[m] = np.zeros((p, len(s_lasso)))
                if len(s_lasso) > 0:
                    for idx, s in enumerate(s_lasso):
                        R[m][s, idx] = 1
...
def mains_contribution(X,
    ...
    range_main = get_ranges4(l1, l2, l3, l4)[0]  # first element
...
def two_ways_contribution(X,gamma_vec,
    ...
    range_2ways = get_ranges4(l1, l2, l3, l4)[1]  # second element
    two_ways_contrib = X[:, range_2ways] @ (beta_vec_2way * gamma_vec)  # elementwise
    return two_ways_contrib


def three_ways_contribution(X,
                            delta_vec,
                            beta_vec_3way,
                            l1=21, l2=14, l3=2, l4=3,
                            already_multiplied=False):
    if already_multiplied:
        delta_vec = np.ones(len(delta_vec))
    range_3ways = get_ranges4(l1, l2, l3, l4)[2]  # third element
    three_ways_contrib = X[:, range_3ways] @ (beta_vec_3way * delta_vec)  # elementwise
    return three_ways_contrib


def four_ways_contribution(X,
                        tau_vec,
                        beta_vec_4way,
                        l1=21, l2=14, l3=2, l4=3,
                        already_multiplied=False):
    if already_multiplied:
        tau_vec = np.ones(len(tau_vec))
    range_4ways = get_ranges4(l1, l2, l3, l4)[3]  # fourth element
    # Ensure column vectors like R's matrix(..., ncol=1)
    tau_vec = tau_vec.reshape(-1, 1)
    beta_vec_4way = beta_vec_4way.reshape(-1, 1)
    four_ways_contrib = X[:, range_4ways] @ (tau_vec * beta_vec_4way)
...
def Q_bern(X,y,beta,gamma_vec,delta_vec,tau_vec,lambda_beta,lambda_gamma,lambda_delta,lambda_tau,
        w_beta,w_gamma=1,w_delta=1,w_tau=1,l1=21,l2=14,l3=2,l4=3,scaled=True,intercept=0):
    """
    Q Bernoulli penalized log likelihood
    """

    if len(beta) == l1 + l2 + l3 + l4:
        # Give beta for main and compute for the rest given gamma delta tau
        beta_2way = get_beta_vec_2way4(beta=beta,
                                    l1=l1, l2=l2, l3=l3, l4=l4,
                                    gamma=gamma_vec,
                                    only_beta=False)
        beta_3way = get_beta_vec_3way4(beta_2way=beta_2way,
                                    l1=l1, l2=l2, l3=l3, l4=l4,
                                    delta=delta_vec,
                                    only_beta=False)
        beta_4way = get_beta_vec_4way4(beta_3way=beta_3way,
                                    tau=tau_vec,
                                    l1=l1, l2=l2, l3=l3, l4=l4,
                                    only_beta=False)
        beta = np.concatenate([beta, beta_2way, beta_3way, beta_4way])

    # Penalty terms
    penalty_beta = get_penalty(vector=beta[get_ranges4(l1, l2, l3, l4)[0]],
                            weights=w_beta,
                            lambda_=lambda_beta)
    ...
    penalty_delta = get_penalty(vector=delta_vec,
                                weights=np.ones_like(delta_vec),
                                lambda_=lambda_delta)
    penalty_tau = get_penalty(vector=tau_vec,
                            weights=np.ones_like(tau_vec),
                            lambda_=lambda_tau)
    ...
    loss = -log_like + penalty_beta + penalty_gamma + penalty_delta + penalty_tau
    return loss



def minimizer_Q_bern_delta(X, y, C, lambda_, beta_old, weight=1, scaled=True):
    def fct(b):
...
def minimizer_Q_bern_gamma(X, Z, y, C, lambda_, beta_old, weight=1, scaled=True):
    def fct(b):
        penalty = abs(b) * lambda_ * weight
        v = X * b + Z * (b ** 2) + C
        log_like = np.sum(y * v - kappa0(v))
...
def minimizer_Q_bern_beta(X, Z, t, y, C, lambda_, beta_old, weight=1, scaled=True):
    def fct(b):
        penalty = weight * abs(b) * lambda_
        v = X * b + Z * (b ** 2) + t * (b ** 6) + C
        log_like = np.sum(y * v - kappa0(v))
        if scaled:
            log_like /= (2 * len(X))
    ...
    interval = (
        min(-beta_old / 5 - 5e-1, 2 * beta_old - 5e-1),
