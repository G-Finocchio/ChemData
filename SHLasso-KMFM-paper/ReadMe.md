...
- `Strong_hierarchical_lasso.egg-info/` — Packaging information auto-generated by `pip install -e .`
...
- `RUN_TO_INSTALL_SHL_package.ipynb` — Notebook that installs the SHL package locally using the command `!pip install -e .`. It will be added 
...
To install the SHL package locally, open the notebook `RUN_TO_INSTALL_SHL_package.ipynb` located in KMFM_main_paper folder and run its single cell. It just runs `!pip install -e .`.

## 🧩 CB SHL — Summary

- The **Strong Hierarchical Lasso (SHL)** models perform penalized hierarchical estimation for factorial problems with up to **five factors**.  
